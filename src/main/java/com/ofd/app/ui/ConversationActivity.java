...
import com.ofd.app.entities.Blockable;
import com.ofd.app.entities.Contact;
import com.ofd.app.entities.Conversation;
import com.ofd.app.entities.Message;
import com.ofd.app.services.XmppConnectionService.OnAccountUpdate;
import com.ofd.app.services.XmppConnectionService.OnConversationUpdate;
import com.ofd.app.services.XmppConnectionService.OnRosterUpdate;
import com.ofd.app.ui.adapter.ConversationAdapter;
import com.ofd.app.utils.ExceptionHelper;
import com.ofd.app.xmpp.OnUpdateBlocklist;
...
public class ConversationActivity extends XmppActivity
	...
	public static final String ACTION_DOWNLOAD = "com.ofd.app.action.DOWNLOAD";
    ...
    private LinearLayout conversationLayout, conversationListLayout;
    ...
    private String deliveryObjId, userType;
...
//	}
	...
	public boolean isConversationsOverviewVisable() {
		if (mContentView instanceof SlidingPaneLayout) {
			SlidingPaneLayout mSlidingPaneLayout = (SlidingPaneLayout) mContentView;
            if(conversationLayout.getLayoutParams().width==0)
                return true;
            else
                return false;
	...
	protected void onCreate(final Bundle savedInstanceState) {
        ...
        conversationListLayout = (LinearLayout)findViewById(R.id.lyt_conv_list);
        conversationLayout = (LinearLayout)findViewById(R.id.selected_conversation);
		...
		if (mContentView instanceof SlidingPaneLayout) {
            ...
            conversationListLayout.getLayoutParams().width = screenWidth;
            conversationListLayout.requestLayout();
            conversationLayout.getLayoutParams().width=0;

            conversationLayout.requestLayout();
...
//			mSlidingPaneLayout.setSliderFadeColor(0);
    ...
    private void showConversationListLayout(){
        conversationListLayout.getLayoutParams().width = screenWidth;
        conversationListLayout.requestLayout();
        conversationLayout.getLayoutParams().width=0;
        conversationLayout.requestLayout();
    ...
    private void showConversationLayout(){
        ...
        userType=appSharedPreferences.getUserType(mSelectedConversation.getContact().getJid().getLocalpart());
        conversationListLayout.getLayoutParams().width = 0;
        conversationListLayout.requestLayout();
        conversationLayout.getLayoutParams().width=screenWidth;
        conversationLayout.requestLayout();
        deliveryObjId=appSharedPreferences.getConversationObjId(mSelectedConversation.getContact().getJid().getLocalpart());
        if(deliveryObjId!=null) {
            if(appSharedPreferences.getConversationIsDelivery(mSelectedConversation.getContact().getJid().getLocalpart())){
                isDelivery = true;
                userType=appSharedPreferences.getUserType(mSelectedConversation.getContact().getJid().getLocalpart());
                ParseQuery<ParseObject> query = ParseQuery.getQuery("Delivery");

                query.getInBackground(deliveryObjId,new GetCallback<ParseObject>() {
                    @Override
                    public void done(ParseObject parseObject, ParseException e) {
                        if(e==null){
                            if(parseObject.get("status").equals("ofd")){
                                if(userType==null){
                                    appSharedPreferences.setConversationAsDelivery(mSelectedConversation.getContact().getJid().getLocalpart(),deliveryObjId,false,"r");
                                    userType="r";
                                }
                            }
                            else{
                                isDelivery=false;
                                appSharedPreferences.setConversationAsDelivery(mSelectedConversation.getContact().getJid().getLocalpart(),deliveryObjId,false,userType);
                            }
                        }
                    }
                });
        ...
        if(appSharedPreferences.getBool("location")){
				AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(ConversationActivity.this);
	    			...
	    			// set dialog message
	    			alertDialogBuilder
	    				.setMessage("Send your Current Location to "+ mSelectedConversation.getJid().getLocalpart()+" ?")
	    				.setCancelable(false)
	    				.setPositiveButton("Confirm",new DialogInterface.OnClickListener() {
	    					public void onClick(DialogInterface dialog,int id) {
	    						dialog.dismiss();
                                sendLocation();
                                appSharedPreferences.setBool("location",false);
	...
	private void updateActionBarTitle() {
		updateActionBarTitle(!isConversationsOverviewVisable());
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
        ...
        final MenuItem menuDeliveryCreate= menu.findItem(R.id.action_create_delivery);
        final MenuItem menuDeliveryReschedule= menu.findItem(R.id.action_schedule);
        final MenuItem menuDeliveryTrack= menu.findItem(R.id.action_track_map);
        final MenuItem menuDeliveryRequestLoc= menu.findItem(R.id.action_location_Request);
		...
		if (isConversationsOverviewVisable()) {
            ...
            menuDeliveryReschedule.setVisible(false);
            menuDeliveryTrack.setVisible(false);
            menuDeliveryRequestLoc.setVisible(false);
		} else {
            if(userType.equals("r")){
                menuDeliveryTrack.setVisible(true);
                menuDeliveryReschedule.setVisible(true);
                menuDeliveryRequestLoc.setVisible(true);
            }
            ...
            menuDeliveryCreate.setVisible(false);
...
//				}
	...
	private void selectPresenceToAttachFile(final int attachmentChoice, final int encryption) {
		if (attachmentChoice == ATTACHMENT_CHOICE_LOCATION && encryption != Message.ENCRYPTION_OTR) {
			...
			Intent intent = new Intent("com.ofd.app.location.request");
				...
				public void onPresenceSelected() {
					...
					switch (attachmentChoice) {
						...
						case ATTACHMENT_CHOICE_LOCATION:
							intent.setAction("com.ofd.app.location.request");
							break;
					}
...
//					break;
                ...
                case R.id.action_create_delivery:
                    Intent deliveryIntent = new Intent(this,CreateDeliveryActivity.class);
                    Conversation conversation = getSelectedConversation();
                    deliveryIntent.putExtra("account", conversation.getAccount().getJid().getLocalpart());
                    startActivity(deliveryIntent);
                    break;
                case R.id.action_track_map:
                    openTrackingActivity();
                    break;
                case R.id.action_location_Request:
                    requestLocation();
                    break;
    ...
    private void requestLocation() {
                 xmppConnectionService.sendMessage(new Message(mSelectedConversation,"Please send me your current location",Message.ENCRYPTION_NONE));
    }
	...
	protected void attachFileDialog() {
		...
		if (new Intent("com.ofd.app.location.request").resolveActivity(getPackageManager()) == null) {
